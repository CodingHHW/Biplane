...
`Biplane` 是一个 3D Slicer Scripted Loadable Module，用于通过三视图（Red/Green/Yellow）上的 2D 标注点反推 3D 空间点，并支持将 3D 点投影回第三视图用于验证，同时提供 “Tracing” 功能将 3D 点实时映射到三视图。
...
- 在 Slicer 内运行，依赖 Slicer 自带的 `vtk` / `numpy` / `SimpleITK` / `OpenCV(cv2)` 等环境
...
7. 在 Red 视图添加 2D 点，选择到 `Red 2D Point`，点击 `redPush`（Green 视图生成约束线）
8. 在 Green 视图添加 2D 点，选择到 `Green 2D Point`，点击 `greenPush`（计算 3D 交点，并在 Yellow 视图显示对应 2D 点）
9. （可选）Tracing：选择 `knife` 点，点击 `real time tracing`，移动 3D 点后会在三视图同步显示投影点

### 输出与文件格式

截图与中间文件默认输出到：Slicer 的临时目录下 `Biplane/`。

每个视图会输出以下文件：

- `shotXBody.png`：仅 Body
- `shotXMarkers.png`：仅 markers
- `shotXTestPoint.png`：仅 testPoint
- `shotX.nii.gz`：合成切片（Body + markers + testPoint）
...
- **TRE (mm)**：选择两组 3D 点，计算欧式距离
- **RE (px)**：选择两组 2D 点，计算像素平面欧式距离
- **Ray Gap (mm)**：两条 3D 光线的最近距离
...
`Biplane` is a 3D Slicer Scripted Loadable Module that reconstructs a 3D target point from 2D markups across multiple slice views (Red/Green/Yellow). It also projects the 3D point back to a third view for validation and provides a “Tracing” feature to continuously map a moving 3D point onto the slice views.
...
9. (Optional) Tracing: select a `knife` markup point, click `real time tracing`, then move the 3D point to see synchronized projections
...
- `shotXBody.png`: body only
- `shotXMarkers.png`: markers only
- `shotXTestPoint.png`: testPoint only
- `shotX.nii.gz`: combined slice (body + markers + testPoint)

Value conventions:

- markers pixel value is `-1000`
- testPoint pixel value is `-100`

### Error Metrics

- **TRE (mm)**: Euclidean distance between two selected 3D points
- **RE (px)**: Euclidean distance between two selected 2D points
